...
export default function SearchableDropdown({
    ...
    required = false,
    disabled = false,
    getOptionValue = (option) => option?.value ?? option?.label ?? option,
    getOptionLabel = (option) => option?.label ?? option?.name ?? getOptionValue(option),
    ...
    const keyword = normalize(showAll ? "" : value);
    const filteredOptions = useMemo(() => {
        const filtered = keyword
            ? mappedOptions.filter((option) => {
                const searchable = normalize(`${option.value} ${option.label} ${option.description}`);
    ...
    const selectOption = (option) => {
        ...
        onSelect?.(option.raw, option.value);
        setShowAll(false);
        setOpen(false);
    };
                ...
                <div className="smart-dropdown absolute left-0 right-0 z-50 mt-2 max-h-64 overflow-y-auto rounded-2xl border border-[#29314b] bg-[#141b2d] p-2 shadow-2xl">
                    {filteredOptions.length > 0 ? filteredOptions.map((option, index) => (
                        <button
                            ...
                            type="button"
                            className="smart-dropdown-item w-full rounded-xl px-3 py-2 text-left transition hover:bg-[#29314b] focus:bg-[#29314b] focus:outline-none"
                            onMouseDown={(event) => event.preventDefault()}
