    ...
    return (
        <AppLayout title="Detail User">
            ...
            <div className="space-y-6">
                <div className="space-y-6">
                    <div className="page-card"><h2 className="mb-4 text-2xl font-black">{item.name}</h2><div className="grid gap-4 md:grid-cols-3"><Info label="Email" value={item.email} /><Info label="Role" value={item.role?.nama_role} /><Info label="Subkategori" value={roleCategory} /><Info label="Jenis Identitas" value={item.jenis_identitas?.nama_jenis ?? item.identity_type} /><Info label="Nomor Identitas" value={item.identity_number} /><Info label="Status" value={item.status} /><Info label="Cabang" value={item.cabang?.nama_cabang} /><Info label="No. HP" value={item.phone} /></div></div>
